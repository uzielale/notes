...
PROCEDURE 1 (Actualizar acumulado evaluacion descendente)
##########################################################################################################################################
DROP PROCEDURE IF EXISTS getAcumuladoEvaluacion;

DELIMITER $$
CREATE PROCEDURE getAcumuladoEvaluacion (_user_id VARCHAR(36), _evaluacion_id VARCHAR(36), _cargo_id VARCHAR(36), _anio INT, _semestre INT)
BEGIN 
...
IF _acumulado > 0 THEN
	...
	END IF;
END IF;
SELECT FORMAT(_acumulado, 2) as acumulado, _cumplimiento_indicador_al, _contador, _acumulado_ind_1, _acumulado_ind_2, _acumulado_ind_3, _acumulado_ind_4, _acumulado_ind_5, _acumulado_ind_6;
CLOSE cursor_indicadores;
END$$

DELIMITER ;

CALL getAcumuladoEvaluacion('5f6f79d7-c90e-11e6-a511-1c6f65990c34', 'd525c1cc-0927-11e9-a65f-000c2916686f', '3e3ed576-be52-11e6-9387-a088694cea32', 2018, 2);
##########################################################################################################################################